...
class Validator:
    ...
    def from_dicts(cls, input_data: dict, result_data: dict):
        ...
        v.vehicle_routes = pd.DataFrame(
            result_data.get("vehicles", []), columns=[NAMES.ID, NAMES.ROUTE, NAMES.TIME]
        )
        v.loader_routes = pd.DataFrame(
            result_data.get("loaders", []), columns=[NAMES.ID, NAMES.ROUTE]
        )
    ...
    def _dist(self, point_id_1, point_id_2):
        return strict_round(
            np.linalg.norm(
                np.array(self.orders.loc[point_id_1, [NAMES.X, NAMES.Y]])
                - np.array(self.orders.loc[point_id_2, [NAMES.X, NAMES.Y]])
            ),
            2,
        )
    ...
    def _proc_result_data(self):
        ...
        if not self.loader_routes.empty:
            veh_routes_orders = (
                self.vehicle_routes[NAMES.ROUTE].sum()
                if not self.vehicle_routes.empty
                else []
            )
            self.loader_routes = (
                self.loader_routes.assign(
                    **{
                        NAMES.ROUTE: lambda df: df[NAMES.ROUTE].apply(
                            lambda x: [_ for _ in x if _ in veh_routes_orders]
                        ),
                    }
                )
                ...
                .assign(
                    **{
                        NAMES.ROUTE: lambda df: df.apply(
                            lambda row: row[NAMES.ROUTE] + [row[NAMES.ROUTE][0]], axis=1
                        ),
                    }
                )
            )
    ...
    def _correct_orders_id(self, routes: pd.DataFrame, name: RouteTypes):
        ...
        if not invalid_id.empty:
            self._logger.warning(
                ...
                + str(
                    invalid_id[[NAMES.ID, NAMES.ROUTE]]
                    .drop_duplicates()
                    .groupby(NAMES.ID)
                    .agg(
                        **{
                            "Invalid orders": pd.NamedAgg(
                                NAMES.ROUTE, lambda x: list(x)
                            )
                        }
                    )
                )
            ...
            if name == "Vehicle":
                self.vehicle_routes[NAMES.TIME] = self.vehicle_routes[
                    [NAMES.TIME, NAMES.ROUTE]
                ].apply(
                    lambda row: [
                        row[NAMES.TIME][i]
                        for i in range(len(row[NAMES.TIME]))
                        if row[NAMES.ROUTE][i + 1] in self.orders.loc[:, NAMES.ID]
                    ],
                    axis=1,
                )
                self.vehicle_routes[NAMES.ROUTE] = self.vehicle_routes[
                    NAMES.ROUTE
                ].apply(lambda x: [_ for _ in x if _ in self.orders.loc[:, NAMES.ID]])
            else:
    ...
    def _correct_start_time(self, routes: pd.DataFrame, name: RouteTypes):
        ...
        if name == "Vehicle":
            ...
            points = (
                ...
                .merge(
                    self.vehicle_routes[[NAMES.ROUTE, NAMES.TIME]]
                ...
                .merge(
                    ...
                    .explode([NAMES.ROUTE, NAMES.TIME])
                    .rename(
                        columns={NAMES.ROUTE: "next_point", NAMES.TIME: "next_time"}
                    ),
                    on=["next_point"],
                    how="left",
                )
                .assign(
                    next_time=lambda df: df.apply(
                        lambda row: (
                            strict_round(
                                self.orders.loc[row["point"], NAMES.LOAD_ST]
                                + row["time"]
                                + strict_round(
                                    self._dist(row["point"], row["next_point"]) / speed,
                                    2,
                                ),
                                2,
                            )
                            if row[NAMES.ROUTE][-1] == row["next_point"]
                            else row["next_time"]
                        ),
                        axis=1,
                    )
        ...
        points = points.merge(
            self.orders[
                [NAMES.ID, NAMES.X, NAMES.Y, service_time_name, NAMES.TIME_WDW]
            ].rename(columns={NAMES.ID: "point_id"}),
            left_on="point",
            right_on="point_id",
            how="left",
        ).merge(
            self.orders[[NAMES.ID, NAMES.X, NAMES.Y]].rename(
                columns={NAMES.ID: "point_id"}
            ),
            left_on="next_point",
            right_on="point_id",
            how="left",
        ...
        points = points.assign(
            ...
            min_correct_time=lambda df: strict_round(
                df.time + df[service_time_name] + df.travel_time, 2
            ),
        ...
        if not unachievable.empty:
            self._logger.warning(
                ...
                + str(
                    unachievable.assign(
                        released=lambda df: df["time"] + df[service_time_name]
                    )[
                        [
                            NAMES.ID,
                            "next_point",
    ...
    def _correct_capacities(self, routes: pd.DataFrame):
        ...
        capacities = (
            ...
            .explode("circles")
            .merge(
                self.orders[[NAMES.ID, NAMES.VOLUME]].rename(
                    columns={NAMES.ID: "circles"}
                ),
                on="circles",
                how="left",
            )
    ...
    def _complete_order(self):
        orders = (
            self.orders.query(f"{NAMES.ID} != 0")
            .merge(
                self.vehicle_routes.explode([NAMES.ROUTE, NAMES.TIME]).rename(
                    columns={NAMES.ID: "veh_id"}
                ),
                how="left",
                left_on=NAMES.ID,
                right_on=NAMES.ROUTE,
            )
            .merge(
                self.loader_routes.explode([NAMES.ROUTE]).rename(
                    columns={NAMES.ID: "loader_id"}
                ),
                how="left",
                left_on=NAMES.ID,
                right_on=NAMES.ROUTE,
        ...
        complete = (
            orders.groupby(
                [NAMES.ID, "veh_id", NAMES.LOADER_CNT, NAMES.OPTIONAL],
                as_index=False,
                dropna=False,
            )
        ...
        if not incomplete_optional.empty:
            ...
            self.costs[OUTPUT.unsch_optional][0] = (
                self.costs[OUTPUT.unsch_optional][1]
    ...
    def _start_and_finish(self):
        ...
        df_loaders = self.loader_routes.assign(
    ...
    def _route_time_calc(self, routes: pd.DataFrame, name: RouteTypes) -> float:
        ...
        if name == "Loader":
            times = (
                ...
                .merge(
                    self.orders[[NAMES.ID, NAMES.LOAD_ST]].rename(
                        columns={NAMES.ID: "end_point"}
                    ),
                    how="left",
                    on="end_point",
                )
    ...
    def _output(self):
        ...
        min_spacing = 3
        max_key_len = max(
            [len(str(k)) for k in self.violations.keys()]
            + [len(str(k)) for k in self.costs.keys()]
        )
        max_val_len = max(
            [len(str(k)) for k in self.violations.values()]
            + [len(str(k[0])) for k in self.costs.values()]
        )
        ...
        self._logger.warning(
            f"Total cost = {strict_round(sum([v[0] for v in self.costs.values()]), 2)}"
        )
    ...
    def validation(self):
        if not self.vehicle_routes[NAMES.ID].is_unique:
            self._logger.warning(
                "Vehicle IDs is not unique. The IDs will be replaced with consecutive ones."
            )
            self.vehicle_routes[NAMES.ID] = np.arange(
                1, self.vehicle_routes.shape[0] + 1
            )

        if (
            not self.vehicle_routes.explode(NAMES.ROUTE)
            .query(f"{NAMES.ROUTE}!=0")[NAMES.ROUTE]
            .is_unique
        ):
        ...
        if not self.loader_routes[NAMES.ID].is_unique:
            self._logger.warning(
                "Loaders IDs is not unique. The IDs will be replaced with consecutive ones."
            )
        ...
        self.costs[OUTPUT.veh_shifts][0] = (
            self.costs[OUTPUT.veh_shifts][1] * self.costs[OUTPUT.veh_shifts][2]
        )
        self.costs[OUTPUT.load_shifts][2] = self.loader_routes.shape[0]
        self.costs[OUTPUT.load_shifts][0] = (
            self.costs[OUTPUT.load_shifts][1] * self.costs[OUTPUT.load_shifts][2]
        )

        self.costs[OUTPUT.veh_fc][2] = self._route_dist_calc(
            self.vehicle_routes, name="Vehicle"
        )
        self.costs[OUTPUT.veh_fc][0] = strict_round(
            self.costs[OUTPUT.veh_fc][1] * self.costs[OUTPUT.veh_fc][2], 2
        )
        self.costs[OUTPUT.load_wt][2] = self._route_time_calc(
            self.loader_routes, name="Loader"
        )
        self.costs[OUTPUT.load_wt][0] = strict_round(
            self.costs[OUTPUT.load_wt][1] * self.costs[OUTPUT.load_wt][2], 2
        )
...
def validate_solution(
    input_data: dict, solution_dict: dict, quiet: bool = True
) -> dict:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dir", default="../data", help="Validate data in dir (default 'data')"
    )
    parser.add_argument(
        "--input_file",
        type=str,
